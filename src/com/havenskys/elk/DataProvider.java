...
public class DataProvider extends ContentProvider {
	...
	public static final Uri CONTENT_URI = Uri
			.parse("content://com.havenskys.elk");
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db2) {
			...
			try {
				...
				SQL += ", title text, summary text, published DATE, content text, author text";
				SQL += ", created DATE";
				SQL += ", updated DATE";
				SQL += ", status INTEGER default 1"; // < 0 deleted(value * -1),
														// 1 active(created), ++
														// per update
				SQL += ");\n";
				db.execSQL(SQL);
				...
				db.execSQL("create index IF NOT EXISTS retrospect_filtered on retrospect(filtered)");
				db.execSQL("create index IF NOT EXISTS retrospect_updated on retrospect(updated)");
				db.execSQL("create index IF NOT EXISTS retrospect_hostname on retrospect(hostname)");
				db.execSQL("create index IF NOT EXISTS retrospect_docpath on retrospect(docpath)");

				db.execSQL("create table IF NOT EXISTS xsaw (_id integer primary key autoincrement, mid integer, content text , updated date, created date, status integer default 1 )");
				db.execSQL("create index IF NOT EXISTS xsaw_mim on xsaw(mid)");
				db.execSQL("create index IF NOT EXISTS xsaw_updated on xsaw(created)");
				db.execSQL("create index IF NOT EXISTS xsaw_updated on xsaw(updated)");
	...
	public boolean onCreate() {
		...
		try {
			...
			if (mDb != null) {
				...
				Cursor d = mDb.query("moment", new String[] { "*" }, "", null,
				...
				if (d == null) {
					...
					if (d.getColumnIndex("visited") == -1) {
						...
						mDb.execSQL("create index IF NOT EXISTS moment_visited on moment(visited);");
					}

					if (d.getColumnIndex("itext") == -1) {
						mDb.execSQL("alter table moment add column itext text");
						mDb.execSQL("create index IF NOT EXISTS moment_itext on moment(itext);");
					}
					if (d.getColumnIndex("contenttype") == -1) {
						mDb.execSQL("alter table moment add column contenttype text");
					}
					if (d.getColumnIndex("language") == -1) {
					...
					if (d.getColumnIndex("mid") == -1) {
						...
						mDb.execSQL("create index IF NOT EXISTS moment_mid on moment(mid);");
					}
					if (d.getColumnIndex("updatemin") == -1) {
						mDb.execSQL("alter table moment add column updatemin integer default 0");
						mDb.execSQL("create index IF NOT EXISTS moment_updatemin on moment(updatemin);");
					}
